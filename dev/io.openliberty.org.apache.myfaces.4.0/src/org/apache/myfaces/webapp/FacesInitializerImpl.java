...
public class FacesInitializerImpl implements FacesInitializer {
    ...
    public void initFaces(ServletContext servletContext) {
        ...
        try {
             ...
             */
            // Parse and validate the web.xml configuration file
            // TODO: This is the old way of doing this copied from the Faces 3.0 AbstractFacesInitializer.
            // The JSFExtensionFactory is what adds the StartupServletContextListener programmatically
            // Doing it the current Faces 4.0 way results in the following exception:
            //[7/27/22, 16:04:00:011 EDT] 00000047 org.apache.myfaces.webapp.FacesInitializerImpl               E An error occured while initializing MyFaces: SRVE9002E: You cannot start this operation from a programmatically added listener when using servlet-3.1 feature. (Operation: getServletRegistrations | Listener: org.apache.myfaces.webapp.StartupServletContextListener | Application: ImportConstantsTag)
            //java.lang.UnsupportedOperationException: SRVE9002E: You cannot start this operation from a programmatically added listener when using servlet-3.1 feature. (Operation: getServletRegistrations | Listener: org.apache.myfaces.webapp.StartupServletContextListener | Application: ImportConstantsTag)
            //        at <unknown class>.getServletRegistrations(WebApp31.java:246)
            //        at <unknown class>.getServletRegistrations(ServletContextFacade.java:657)
            //        at <unknown class>.getServletRegistrations(FacesServletMappingUtils.java:121)
            //        at <unknown class>.getFacesServletRegistration(FacesServletMappingUtils.java:152)
            //        at <unknown class>.initFaces(FacesInitializerImpl.java:154)
            //        at <unknown class>.contextInitialized(StartupServletContextListener.java:54)
            //        at <unknown class>.notifyServletContextCreated(WebApp.java:2447)
            //        at <unknown class>.notifyServletContextCreated(WebApp31.java:517)
        ...
        try {
            ...
            if (log.isLoggable(Level.FINE)) {
                log.log(Level.FINE, "An error occured while instantiating a new ExpressionFactory. "
                                    + "Attempted to load class '" + expressionFactoryClassName + "'.",
                        ex);
            }
        }
    ...
    private FacesContext _createFacesContext(ServletContext servletContext, boolean startup) {
        ...
        FacesContext facesContext = new StartupFacesContextImpl(externalContext, externalContext, exceptionHandler, startup);
    ...
    public void _callPreDestroyOnInjectedJSFArtifacts(FacesContext facesContext) {
        ...
        List<BeanEntry> injectedBeanStorage = (List<BeanEntry>) facesContext.getExternalContext().getApplicationMap().get(INJECTED_BEAN_STORAGE_KEY);
    ...
    protected void initWebsocketIntegration(ServletContext servletContext, ExternalContext externalContext) {
        ...
        if (Boolean.TRUE.equals(b)) {
            ...
            try {
                serverContainer.addEndpoint(ServerEndpointConfig.Builder.create(EndpointImpl.class,
                                                                                EndpointImpl.JAKARTA_FACES_PUSH_PATH).configurator(new WebsocketConfigurator(externalContext)).build());
    ...
    private void dispatchInitializationEvent(ServletContext context, int operation) {
        ...
        List<StartupListener> pluginEntries = (List<StartupListener>) context.getAttribute(MyfacesConfig.FACES_INIT_PLUGINS);
