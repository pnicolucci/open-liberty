...
public class WebXml {
    ...
    private final Map _servlets = new HashMap();
    private final Map _servletMappings = new HashMap();
    private final Map _filters = new HashMap();
    private final Map _filterMappings = new HashMap();
    ...
    public List getFacesExtensionsFilterMappings() {
        ...
        for (Iterator it = _filters.entrySet().iterator(); it.hasNext();) {
            ...
            if (urlPatterns != null) {
                for (Iterator it2 = urlPatterns.iterator(); it2.hasNext();) {
                    String urlpattern = (String) it2.next();
                    tempExtensionsFilterMappings.add(new org.apache.myfaces.shared_impl.webapp.webxml.FilterMapping(filterName, filterClass, urlpattern));
                    if (log.isLoggable(Level.FINEST)) {
