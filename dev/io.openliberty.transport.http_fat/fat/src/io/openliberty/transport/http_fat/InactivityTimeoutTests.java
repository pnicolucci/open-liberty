...
public class InactivityTimeoutTests {
     ...
     *
     * The test will sleep for 11s which is 2X + 1 greater than the inactivityTimeout.
    ...
    public void testInactivityTimeout_one_request() throws Exception {
        ...
        try (Socket socket = new Socket(url.getHost(), url.getPort())) {
            ...
            // to ensure the inactivtyTimeout is invoked.
        ...
        assertTrue("The SocketTimeoutException was not found in the trace and should have been!", server.findStringsInLogsAndTraceUsingMark("SocketTimeoutException").size() == 1);
    }
    ...
    public void testInactivityTimeout_two_requests() throws Exception {
        ...
        try (Socket socket = new Socket(url.getHost(), url.getPort())) {
            ...
            assertTrue("The expected response: " + expectedResponse1 + " was not received!", testPassed);
            ...
            while ((line = reader.readLine()) != null) {
                LOG.info(line);
                testFailed = true; // There should be no response!
            }
        }

        assertFalse("There was no response expected but one was received", testFailed);
     ...
     * only the 6s sleep vs the 11s sleep where it would be expected to have a "HTTP/1.1 408 Request Timeout" response or a SocketException.
     *
     *
     * @throws Exception
     */
    ...
    public void testInactivityTimeout_verify_read_retry() throws Exception {
        ...
        try (Socket socket = new Socket(url.getHost(), url.getPort())) {
            ...
            // Wait for "TCPReadReques 1   read (async) requested for local: localhost/127.0.0.1:8010 remote: localhost/127.0.0.1:36762" and start sleeping after
            // to ensure the inactivtyTimeout is not invoked.
            server.waitForStringInTraceUsingMark(".*read \\(async\\) requested for local");
        ...
        assertTrue("The expected response: " + expectedResponse + " was not received!", testPassed);
    }
}
